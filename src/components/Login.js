...
const Login = () => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Note: Navigation happens in the onAuthStateChanged listener
    } catch (error) {
      setErrorMessage(error.code + " - " + error.message);
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (user) {
        ...
        // setLoading(false) will be called after navigation completes
  ...
  // Handle loading state after navigation
  useEffect(() => {
      ...
      {loading && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
          <div className="bg-white p-5 rounded-lg flex flex-col items-center">
            <div className="w-12 h-12 border-4 border-gray-300 border-t-4 border-t-gray-700 rounded-full animate-spin mb-3"></div>
            <p className="text-gray-800 font-semibold">Signing in...</p>
          </div>
        </div>
      )}
      ...
      <div className="relative z-10 w-full max-w-md px-4">
        ...
        >
          ...
          >
            {loading ? 
              <span className="flex items-center">
                <span className="w-5 h-5 border-2 border-gray-700 border-t-transparent rounded-full animate-spin mr-2"></span>
                Please wait...
              </span> : 
              <>
                <svg className="w-5 h-5" viewBox="0 0 24 24">
